...
const CFQuery = () => {
  ...
  const queryProblem = async () => {
    ...
    const result = await recommendProblem({
      tags: tags.join(";"),
      rating: queryRating,
    });
    setIsLoading(false);
  ...
  const XPill: FunctionComponent<
    PropsWithChildren<{ tag: string; index: number }>
  > = ({ tag, index }) => {
    return (
      <span
              ...
              {tagsPopup > 0 &&
                allTags.map(
                  (tag, index) =>
                    inSearch(tag) && (
                      <span
                        className={`transition-colors duration-300 select-none cursor-pointer rounded-full px-3 py-1 ${
                          tags.includes(tag)
